...
class BeDancer():
    ...
    def cos_sim(self, dance_vectors, user_vectors):
        print(np.norm(dance_vectors))
        print('hhhhhhhhh')
        return np.dot(dance_vectors, user_vectors) / (np.norm(dance_vectors) * np.norm(user_vectors))

    def vector(self,landmarks):
    ...
    def accuracy_thread(self):
        ...
        while continue_window == True :
            ...
            with self.mp_pose.Pose(model_complexity=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
                ...
                dance_vectors = self.vector(dance_landmarks_thread)
                user_vectors = self.vector(user_landmarks_thread)
                accuracy = math.ceil((np.sum((dance_vectors * user_vectors) / (norm(dance_vectors) * norm(user_vectors))) + 1) * 50)
                
                thread_dancerANDuser_with_landmarks =np.hstack((dance_image_thread, user_image_thread))
                cv2.putText(thread_dancerANDuser_with_landmarks, 'Score : '+str(accuracy), (20, 50), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=2, color=(0, 0, 255), thickness=2, lineType=cv2.LINE_AA)
                ...
                # accuracy = self.cos_sim(self.vector(dance_landmarks_thread),self.vector(user_landmarks_thread)) * 100
